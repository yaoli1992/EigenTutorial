...
void Problem::AddOrderingSLAM(std::shared_ptr<backend::Vertex> v) {
    if (IsPoseVertex(v)) {
...
bool Problem::IsPoseVertex(std::shared_ptr<backend::Vertex> v) {
    string type = v->TypeInfo();
    return type == string("VertexPose") ||
            type == string("VertexSpeedBias");
}

bool Problem::IsLandmarkVertex(std::shared_ptr<backend::Vertex> v) {
    string type = v->TypeInfo();
